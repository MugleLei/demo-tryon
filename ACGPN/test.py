    ...
    for i, data in enumerate(dataset):
        ...
        for j in range(opt.batchSize):
            ...
            util.save_tensor_as_image(fake_image[j],
                                      os.path.join(opt.results_dir, 'try-on_'+data['name'][j]))
            util.save_tensor_as_image(warped_cloth[j],
                                      os.path.join(opt.results_dir, 'warped-cloth_'+data['name'][j]))
            util.save_tensor_as_image(refined_cloth[j],
                                      os.path.join(opt.results_dir, 'refined-cloth_'+data['name'][j]))
